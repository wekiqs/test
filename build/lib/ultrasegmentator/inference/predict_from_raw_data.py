...
from ultrasegmentator.inference.data_iterators import PreprocessAdapterFromNpy, preprocessing_iterator_fromfiles, \
    preprocessing_iterator_fromnpy
from ultrasegmentator.inference.export_prediction import export_prediction_from_logits, \
    convert_predicted_logits_to_segmentation_with_correct_shape
from ultrasegmentator.inference.sliding_window_prediction import compute_gaussian, \
    compute_steps_for_sliding_window
from ultrasegmentator.utilities.file_path_utilities import get_output_folder, check_workers_alive_and_busy
...
from ultrasegmentator.utilities.json_export import recursive_fix_for_json_export
from ultrasegmentator.utilities.label_handling.label_handling import determine_num_input_channels
from ultrasegmentator.utilities.plans_handling.plans_handler import PlansManager, ConfigurationManager
from ultrasegmentator.utilities.utils import create_lists_from_splitted_dataset_folder
...
class nnUNetPredictor(object):
    ...
    def initialize_from_path(self, cache_dir: str):
        """
        This is used when making predictions with a trained model
        """

        dataset_json = load_json(join(cache_dir, 'dataset.json'))
        plans = load_json(join(cache_dir, 'plans.json'))
        plans_manager = PlansManager(plans)

        parameters = []
        checkpoint = torch.load(os.path.join(cache_dir, "nnUNet.pth"), map_location=torch.device('cpu'), weights_only=False)
        ...
        trainer_class = recursive_find_python_class(join(ultrasegmentator.__path__[0], "training", "nnUNetTrainer"),
                                                    trainer_name, 'ultrasegmentator.training.nnUNetTrainer')
        if trainer_class is None:
            raise RuntimeError(f'Unable to locate trainer class {trainer_name} in ultrasegmentator.training.nnUNetTrainer. '
...
def predict_entry_point_modelfolder():
    ...
    parser = argparse.ArgumentParser(description='Use this to run inference with nnU-Net. This function is used when '
                                                 ...
                                                 'model. This is useful when the ultrasegmentator environment variables '
...
if __name__ == '__main__':
    ...
    predictor.initialize_from_trained_model_folder(
        join(nnUNet_results, 'Dataset130_MICCAI_challenge/nnUNetTrainer__nnUNetPlans__3d_fullres'),
        use_folds=(0,),
        checkpoint_name='checkpoint_final.pth',
    )
    predictor.predict_from_files(join(nnUNet_raw, 'Dataset134_MICCAI_Validation/imagesTs'),
                                 join(nnUNet_raw, 'Dataset134_MICCAI_Validation_v1'),
                                 save_probabilities=False, overwrite=False,
                                 num_processes_preprocessing=2, num_processes_segmentation_export=2,
                                 folder_with_segs_from_prev_stage=None, num_parts=1, part_id=0)

    # predict a numpy array
    # from ultrasegmentator.imageio.simpleitk_reader_writer import SimpleITKIO
