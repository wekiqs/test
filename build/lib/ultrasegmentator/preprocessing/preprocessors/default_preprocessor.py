...
from ultrasegmentator.paths import nnUNet_preprocessed, nnUNet_raw
from ultrasegmentator.preprocessing.cropping.cropping import crop_to_nonzero
from ultrasegmentator.preprocessing.resampling.default_resampling import compute_new_shape
...
from ultrasegmentator.utilities.utils import get_filenames_of_train_images_and_targets
...
class DefaultPreprocessor(object):
    ...
    def run_case_save(self, output_filename_truncated: str, image_files: List[str], seg_file: str,
        ...
        os.makedirs(output_filename_truncated[:output_filename_truncated.rfind('/')], exist_ok=True)
    ...
    def _normalize(self, data: np.ndarray, seg: np.ndarray, configuration_manager: ConfigurationManager,
        ...
        for c in range(data.shape[0]):
            ...
            normalizer_class = recursive_find_python_class(join(ultrasegmentator.__path__[0], "preprocessing", "normalization"),
                                                           scheme,
                                                           'ultrasegmentator.preprocessing.normalization')
    ...
    def run(self, dataset_name_or_id: Union[int, str], configuration_name: str, plans_identifier: str,
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes) as p:
            for k in dataset.keys():
                output_file_npz = join(output_directory, k + '.npz')
                if os.path.isfile(output_file_npz):
                    print(f"Skipping {k} because output file already exists.")
                    continue
                data_npy = output_file_npz[:-3] + "npy"
                seg_npy = output_file_npz[:-4] + "_seg.npy"
                
                try:
                    data = np.load(data_npy, mmap_mode='r')
                    seg_data = np.load(seg_npy, mmap_mode='r')
                    continue
                    ...
                    remaining.append(len(r) - 1)
            # p is pretty nifty. If we kill workers they just respawn but don't do any work.
            # So we need to store the original pool of workers.
            workers = [j for j in p._pool]
            with tqdm(desc=None, total=len(remaining), disable=self.verbose) as pbar:
