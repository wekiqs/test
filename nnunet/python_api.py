...
from nnunet.inference.predict_from_raw_data import *
...
def download_file(url: str, filename: str) -> Path:
    ...
    """
    # 创建缓存目录（如果不存在）
    cache_dir = get_cache_dir() / "my_app_cache"  # 添加应用专属子目录
...
def ultrasegmentator(input: Union[str, Path], output: Union[str, Path, None] = None,tile_step_size=0.5, use_gaussian=True, use_mirroring=True,
    ...
    # 加载预训练模型
    predictor.initialize_from_path(output, model_path=model_path)
